...
export default async function getFlightData(days){
  ...
  const netherlands = "eyJzIjoiQU1TIiwiZSI6Ijk1NTY1MDQ0IiwiaCI6IjI3NTM2NTYxIn0="
...
const options = {
  ...
  function sortingFlights(country){
    ...
    const lowestDuration = country?.data?.itineraries.filter((item) => item.legs[0].durationInMinutes < 530 && item.legs[1].durationInMinutes < 530 )
...
try {
  ...
  const response = await Promise.allSettled([promise, promise2, promise3, promise4])
  const data = {bel: await response[0].value.json(), ned: await response[1].value.json(), fra: await response[2].value.json(), ger: await response[3].value.json()}
  ...
  return [...netherlandsFlights, ...belgiumFlights, ...franceFlights, ...germanyFlights]
...
export async function uploadFile(supabase) {
  ...
  const { data, error } = await supabase.storage.from('images-flight').update('/flights12', file, 
  ...
  export async function deleteAll(supabase){
    try {
      const {data, error} = await supabase.from("flights").delete().eq("originName", "BRU")
